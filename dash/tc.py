class Tc:
    def __init__(self, hmsf, units, fr=None):
        self._units = units
        self._fr = fr
        self.hmsf = hmsf # string hh:mm:ss.sss in hms-format, hh:mm:ss:ff
    ...
    @hmsf.setter
    def hmsf(self, hmsf_human): # string hh:mm:ss.sss in hms-format, hh:mm:ss:fff with f at the end in smpte-format
        if self.units == 'smpte':
            hmsf_split = hmsf_human.split(':')
            ...
            ss = int(hmsf_split[2])
            ff = int(hmsf_split[3])
            hmsf_formated = '{:02d}:{:02d}:{:02d}:{:02d}'.format(hh, mm, ss, ff)
        else:
            hmsf_split = hmsf_human(':')
            hmsf_len = len(hmsf_split)
            ...
            hmsf_formated = '{:02d}:{:02d}:{:.03f}'.format(hh, mm, ss)
        self._hmsf = hmsf_formated
    ...
    def seconds(self): # calculates seconds
        hmsf_split = [float(i) for i in self.hmsf.split(':')]
...
#         self.call_frames()
#
#     def __units_extractor(self): # checks if the hmsf string has f at the end - then smpte
#         if self._hmsf[-1] == 'f':
#             self._units = 'smpte'
#         else:
#             self._units = 'hms'
